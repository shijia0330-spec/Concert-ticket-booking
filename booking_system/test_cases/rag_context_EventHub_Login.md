...
- **检索词**：EventHub — Login (observed UI scope) 测试用例 业务规则 验收标准 边界
- **命中条数**：5
...
3. [bug] bug sample seckill redis - 预期结果: bug sample seckill redis 预期结果 
- 应返回 503 或 400，JSON 中明确提示「缓存服务不可用，无法预热库存」。
- 不应将堆栈信息直接暴露给 API 调用方。
